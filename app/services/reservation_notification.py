...
class ReservationNotificationService:
    ...
    async def deliver_creation(self, reservation_id: uuid.UUID) -> None:
        """Load the reservation + user and send the booking-confirmation DM.

        Used by the admin Create-Reservation flow. Never raises for a delivery
        failure; the reservation has already committed. Skips users flagged as
        having blocked the bot.
        """
        reservation = await self._res_repo.get_reservation_with_details(reservation_id)
        if reservation is None:
            logger.warning(
                "reservation_create_notify_missing", reservation_id=str(reservation_id)
            )
            return
        ...
        text = self._format_creation(reservation)
        success, error = await self._notif.send(user.telegram_id, text)

        if success:
            logger.info(
                "reservation_create_notify_sent",
                reservation_id=str(reservation_id),
                user_id=str(user.id),
            )
        else:
            logger.warning(
                "reservation_create_notify_failed",
                reservation_id=str(reservation_id),
                user_id=str(user.id),
                error=error,
            )
    ...
    @staticmethod
    def _format_creation(reservation: Reservation) -> str:
        """HTML-safe confirmation message for an admin-created reservation."""
        slot_local = reservation.slot.slot_datetime.astimezone(TZ)
        channel_name = html.escape(reservation.channel.name)

        text = (
            "🎉 <b>Reservation Confirmed</b>\n\n"
            "A reservation has been created for you by the support team.\n\n"
            f"📅 Date: <b>{slot_local.strftime('%A, %d %B %Y')}</b>\n"
            f"🕐 Time: <b>{slot_local.strftime('%I:%M %p')}</b>\n"
            f"📡 Channel: {channel_name}"
        )
        invite_link = reservation.channel.invite_link
        if invite_link:
            text += f'\n🔗 <a href="{html.escape(invite_link)}">Join Channel</a>'
        text += "\n\nYou can view or cancel this under <b>My Reservations</b>."
        return text
